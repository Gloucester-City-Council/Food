...
async function generateOwnerReport(inspectionId) {
  ...
  const premises = await database.getPremises(inspection.premises_ref);
  if (!premises) throw new Error(`Premises not found: ${inspection.premises_ref}`);

  const council = config.reports.council;
  const total = (inspection.hygienic_score || 0) + (inspection.structure_score || 0) + (inspection.management_score || 0);
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @media print { body { background: white; } .report { box-shadow: none; margin: 0; } .report-header { background: none; color: black; border-bottom: 3px solid black; } }
  </style>
</head>
<body>
  <div class="report">
    ...
    <div class="report-body">
      ...
      <div class="section">
        ...
        <div class="rating-box">
          ...
          <p style="margin-top:10px;font-size:13px;color:#555;">Food Hygiene Rating Scheme (FHRS) rating awarded under the Food Standards Agency Brand Standard</p>
        </div>
      </div>
      <div class="section">
        <div class="section-title">Inspection Score Breakdown</div>
        <p style="font-size:13px;color:#555;margin-bottom:10px;">Scores are awarded based on the Food Law Code of Practice risk assessment scheme. Lower scores indicate better compliance.</p>
        <table class="score-table">
          <thead><tr><th>Assessment Area</th><th style="text-align:center;">Score</th><th style="text-align:center;">Maximum</th></tr></thead>
          <tbody>
            <tr><td>Hygienic Food Handling</td><td class="score-cell">${inspection.hygienic_score != null ? inspection.hygienic_score : '-'}</td><td class="score-cell">25</td></tr>
            <tr><td>Cleanliness and Condition of Facilities &amp; Building</td><td class="score-cell">${inspection.structure_score != null ? inspection.structure_score : '-'}</td><td class="score-cell">25</td></tr>
            <tr><td>Management of Food Safety</td><td class="score-cell">${inspection.management_score != null ? inspection.management_score : '-'}</td><td class="score-cell">30</td></tr>
            <tr class="total-row"><td>Total Score</td><td class="score-cell">${total}</td><td class="score-cell">80</td></tr>
          </tbody>
        </table>
      </div>
      ${enforcementList.length > 0 ? `<div class="section"><div class="section-title">Enforcement Action</div><div class="actions-box ${enforcementList.some((e) => e.includes('Prohibition') || e.includes('Closure')) ? 'serious' : ''}">${enforcementList.map((e) => `<div class="enforcement-item">&bull; ${formatEnforcement(e)}</div>`).join('')}</div></div>` : ''}
      ${inspection.actions_required ? `<div class="section"><div class="section-title">Actions Required</div><div class="actions-box"><p style="font-size:14px;">${inspection.actions_required.replace(/\n/g, '<br>')}</p></div>${inspection.revisit_required ? `<p style="margin-top:10px;font-size:14px;"><span class="important">A revisit inspection is required.</span>${inspection.revisit_date ? ` Scheduled for: <strong>${formatDate(inspection.revisit_date)}</strong>` : ''}</p>` : ''}</div>` : ''}
      ${inspection.additional_notes ? `<div class="section"><div class="section-title">Additional Observations</div><p style="font-size:14px;">${inspection.additional_notes.replace(/\n/g, '<br>')}</p></div>` : ''}
      <div class="section">
        ...
        <div class="appeal-box">
          ...
          <ul style="margin:8px 0 8px 20px;font-size:13px;">
            <li><strong>Appeal</strong> – You may appeal the rating within 21 days.</li>
            <li><strong>Request a Re-visit</strong> – If you have made improvements since the inspection.</li>
            <li><strong>Right to Reply</strong> – You can submit a comment published alongside your rating.</li>
          </ul>
      ...
      <div class="section">
        ...
        <p style="font-size:13px;color:#555;">This inspection was carried out under powers conferred by the Food Safety Act 1990, the Food Hygiene (England) Regulations 2013, and Regulation (EC) No 852/2004.</p>
      </div>
      <div class="section">
        <div class="section-title">Contact Us</div>
        <div class="contact-box">
          <p><strong>${council.department}</strong></p>
          <p>${council.name}</p><p>${council.address}</p>
          <p>Telephone: ${council.telephone}</p><p>Email: ${council.email}</p>
...
async function createAndSaveReport(inspectionId) {
  const html = await generateOwnerReport(inspectionId);
  const inspection = await database.getInspection(inspectionId);
  await database.saveOwnerReport(inspectionId, inspection.premises_ref, html);
  return html;
}
...
function formatInspectionType(type) {
  return { routine: 'Routine Inspection', followup: 'Follow-up Inspection', complaint: 'Complaint Investigation', new_business: 'New Business Registration', revisit: 'Re-visit' }[type] || type || 'Routine Inspection';
}
function getRatingLabel(rating) {
  return { 5: 'Very Good', 4: 'Good', 3: 'Generally Satisfactory', 2: 'Improvement Necessary', 1: 'Major Improvement Necessary', 0: 'Urgent Improvement Required' }[rating] || 'Not Yet Rated';
}
function getRatingColor(rating) {
  return { 5: '#1b5e20', 4: '#33691e', 3: '#f57f17', 2: '#e65100', 1: '#bf360c', 0: '#b71c1c' }[rating] || '#6c757d';
}
function formatEnforcement(action) {
  return { written_warning: 'Written Warning Issued', improvement_notice: 'Hygiene Improvement Notice Served', emergency_prohibition: 'Emergency Prohibition Notice Served', voluntary_closure: 'Voluntary Closure Agreed', none: 'No Enforcement Action Required' }[action] || action;
}

module.exports = { generateOwnerReport, createAndSaveReport };
